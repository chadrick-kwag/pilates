...
function Container({ onSlotClicked, targetDate }) {
    ...
    const { loading, error, data } = useQuery(QUERY_LESSON_WITH_TIMERANGE_BY_INSTRUCTOR_PERSONID, {
        client: client,
        fetchPolicy: 'no-cache',
        variables: _var,
        onCompleted: d => {
            console.log(d)
            if (d.query_lesson_with_timerange_by_instructor_personid.success) {

                let pd = d.query_lesson_with_timerange_by_instructor_personid.lessons.map(a => {
